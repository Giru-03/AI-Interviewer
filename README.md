...
2.  **Backend**: A FastAPI server manages the interview logic.
    -   **Session Management**: Maintains conversation history and context using `InMemoryChatMessageHistory`.
...
-   **Stateless-ish Backend**: While the backend uses `InMemoryChatMessageHistory`, it's designed to be easily adaptable to a database-backed session store for scalability.
